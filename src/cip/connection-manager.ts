...
import { encodeTimeout } from './unconnected-send';

/** Redundant Owner — Vol 1, Table 3-5.8 Field 15 */
export const Owner = { Exclusive: 0, Multiple: 1 } as const;

/** Connection Type — Vol 1, Table 3-5.8 Fields 14-13 */
export const ConnectionType = { Null: 0, Multicast: 1, PointToPoint: 2 } as const;

/** Priority — Vol 1, Table 3-5.8 Fields 11-10 */
export const Priority = { Low: 0, High: 1, Scheduled: 2, Urgent: 3 } as const;

/** Fixed/Variable — Vol 1, Table 3-5.8 Field 9 */
export const FixedVar = { Fixed: 0, Variable: 1 } as const;

/** Timeout Multiplier lookup — Vol 1, Section 3-5.4.1.4 */
const TIMEOUT_MULTIPLIER: Record<number, number> = {
  4: 0,
  8: 1,
  16: 2,
  32: 3,
 ...
 *   8-0:   Connection Size
 */
export function encodeConnectionParams(
  owner: number,
  type: number,
  priority: number,
  fixedVar: number,
...
export function buildForwardOpenData(opts: ForwardOpenOptions = {}): Buffer {
  ...
  const timeout = encodeTimeout(timeoutMs);
 ...
 */
export function buildForwardCloseData(opts: ForwardCloseOptions): Buffer {
  const timeoutMs = opts.timeoutMs ?? 1000;
  const vendorId = opts.vendorId ?? 0x3333;
  const originatorSerial = opts.originatorSerial ?? 0x1337;
